...
def _orient(nodes: npt.ArrayLike, orient: int):
    """
...
class KeyFrame:
    ...
    """
    def __init__(self, position: npt.ArrayLike, rotation: float):
        self.position = np.asarray(position, dtype=np.float32)
        self.rotation = rotation
    
    def get_rotation_matrix(self) -> npt.NDArray[np.float32]:
        c = cos(self.rotation)
        s = sin(self.rotation)
...
class Animation:
    ...
    @staticmethod
    def static(position: npt.ArrayLike = (0, 0), angle: float = 0):
        """
    ...
    @staticmethod
    def periodic(p1: npt.ArrayLike, p2: npt.ArrayLike, freq: float, angle: float = 0, offset: float = 0):
        """
...
class Obstacle:
    ...
    """
    _nodes: npt.NDArray[np.float32]
    _padded_nodes: Union[npt.NDArray[np.float32], None] = None
    _keyframe: KeyFrame
    ...
    @property
    def nodes(self) -> npt.NDArray[np.float32]:
        return self._nodes

    @nodes.setter
    def nodes(self, nodes: npt.ArrayLike) -> None:
        self._nodes = np.asarray(nodes, dtype=np.float32)
        self._padded_polygon = None
        self._padded_nodes = None

    @property
    def padded_nodes(self) -> npt.NDArray[np.float32]:
        # padded_nodes is a cached property
        if self._padded_nodes is None:
            polygon = Polygon(self.nodes)
            self._padded_nodes = _exterior_nodes(polygon.buffer(ATR.RADIUS, join_style=JOIN_STYLE.round, resolution=4), 1)
    ...
    def get_mitred_vertices(self) -> npt.NDArray[np.float32]:
        """
        Returns the coordinates of the corners of the shape of this obstacle,
        padded by the ATR radius (mitred padding)
        """
        polygon = Polygon(self.get_vertices())
        return _exterior_nodes(polygon.buffer(ATR.RADIUS, join_style=JOIN_STYLE.mitre, mitre_limit=2), 1)
    ...
    def get_padded_vertices(self) -> npt.NDArray[np.float32]:
        """
        Returns the coordinates of the corners of this obstacle padded by the
        ATR radius at the current simulation time (i.e. including
    ...
    # @staticmethod
    # def create_mpc_dynamic(p1: npt.ArrayLike, p2: npt.ArrayLike, freq: float, rx: float, ry: float, angle: float, corners: int = 12) -> 'Obstacle':
    ...
    @staticmethod
    def create_mpc_dynamic(p1: npt.ArrayLike, p2: npt.ArrayLike, freq: float, rx: float, ry: float, angle: float, corners: int = 12, random: bool = True) -> 'Obstacle':
        """Creates a dynamic obstacle according to the MPC paper https://doi.org/10.1109/CASE49439.2021.9551644
        ...
        if random:
            if np.random.choice([True, False]):
                ...
                scaling_factor = 1
                nodes = np.random.choice([u_shape, l_shape])*scaling_factor
                return Obstacle(nodes, False, Animation.periodic(p1, p1, freq, angle, offset=0.5 * pi / freq))
    ...
    @staticmethod
    def create_non_convex_u_shape(p1: npt.ArrayLike, p2: npt.ArrayLike, freq: float, rx: float, ry: float, angle: float) -> 'Obstacle':
...
class Boundary:
    ...
    _vertices: npt.NDArray[np.float32]
    _padded_polygon: Union[Polygon, None] = None
    ...
    @vertices.setter
    def vertices(self, vertices: npt.ArrayLike) -> None:
    ...
    def padded_polygon(self) -> Polygon:
        ...
        if self._padded_polygon is None:
            ...
            self._padded_polygon = polygon.buffer(-ATR.RADIUS, join_style=JOIN_STYLE.round, resolution=4)
        return self._padded_polygon
